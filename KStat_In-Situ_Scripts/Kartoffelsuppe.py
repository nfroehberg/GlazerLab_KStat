...
if __name__ == "__main__":
    ...
    with serial.Serial(voltammetry_port, 9600, timeout=1) as ser2:
        log('start measurement {}'.format(profile_id))
        
        while True:
            depth = depth + 1
            beep(0.2,0)
            vlt_file, p, t, s, pH, pH_flag = measurement(ser2)

            # Data output
            ts = time.time()
            write_data(depth, ts, vlt_file, p, t, s, pH, pH_flag)
            log("data written")

                               
        log('completed measurement {}'.format(profile_id))
