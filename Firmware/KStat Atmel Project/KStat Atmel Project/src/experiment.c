...
static void pmt_idle(uint16_t idle_volts);
...
void experiment_handler(char command[]){
    ...
    switch (command[0]){
        ...
        case 'M': //PMT idle mode - holds voltage at specified value with no data output
            sscanf(command+1, "%i",&idle_volts);
			pmt_idle(idle_volts);
            break;
...
void pmt_idle(uint16_t idle_volts){
    max5443_set_voltage1(idle_volts);
    ioport_set_pin_level(PIN_POT_2ELECTRODE, 1);
    ioport_set_pin_level(PIN_POT_CE, 1);
    ioport_set_pin_level(PIN_POT_WE, 1);
    ioport_set_pin_level(PIN_POT_CTRL, 1);
